...
public class MergeOnReadInputFormat
  ...
  private InternalSchema internalSchema1;
  ...
  private MergeOnReadInputFormat(
    ...
    this.schemaEvolutionContext = new SchemaEvolutionContext(conf);
  }
  ...
  public void open(MergeOnReadInputSplit split) throws IOException {
    ...
    boolean onlyBaseFile = !(split.getLogPaths().isPresent() && split.getLogPaths().get().size() > 0);
    RowDataProjection projection = null;
    CastMap castMap = null;
    if (schemaEvolutionContext.isEnabled()) {
      long commitTime = Long.parseLong(split
              .getBasePath()
              .map(FSUtils::getCommitTime)
              .orElse(split.getLatestCommit()));
      this.internalSchema = schemaEvolutionContext.getMergedSchema(commitTime);
      this.internalSchema1 = schemaEvolutionContext.getInternalSchema();
      this.actualFieldNames = internalSchema.columns().stream().map(Types.Field::name).collect(Collectors.toList());
      String tableName = schemaEvolutionContext.getTableName();
      this.actualFieldTypes = AvroSchemaConverter.convertToDataType(AvroInternalSchemaConverter.convert(internalSchema, tableName)).getChildren();
      castMap = CastMap.of(tableName, schemaEvolutionContext.getQuerySchema(), internalSchema);
      boolean baseFileWithCommitTime = onlyBaseFile && split.getInstantRange().isPresent();
      List<Integer> selectedFields = Arrays.stream(this.requiredPos).boxed().collect(Collectors.toList());
      List<LogicalType> readType = new ArrayList<>(selectedFields.size());
      for (int pos : selectedFields) {
        readType.add(actualFieldTypes.get(pos).getLogicalType());
      }
      LogicalType[] requiredRowType = readType.toArray(new LogicalType[0]);
      int[] positions = IntStream.range(0, selectedFields.size()).toArray();
      if (castMap.containsAnyPos(selectedFields)) {
        int[] pos = positions;
        if (baseFileWithCommitTime) {
          pos = Arrays.stream(positions).map(i -> i + 1).toArray();
        }
        if (!onlyBaseFile && split.getBasePath().isPresent() && split.getMergeType().equals(FlinkOptions.REALTIME_PAYLOAD_COMBINE)) {
          projection = new RowDataCastProjection(
                  requiredRowType,
                  requiredPos,
                  castMap.rearrange(selectedFields, Arrays.stream(positions).boxed().collect(Collectors.toList()))
          );
        } else {
          projection = new RowDataCastProjection(
                  requiredRowType,
      ...
      } else if (baseFileWithCommitTime) {
        projection = RowDataProjection.instance(requiredRowType, Arrays.stream(positions).map(i -> i + 1).toArray());
      }
    }
    ...
    if (onlyBaseFile) {
      if (split.getInstantRange().isPresent()) {
        ...
        if (projection == null) {
          int[] positions = IntStream.range(1, 1 + tableState.getRequiredRowType().getFieldCount()).toArray();
          projection = RowDataProjection.instance(tableState.getRequiredRowType(), positions);
        }
      } else {
        // base file only
      ...
      RecordIterator baseIterator = new BaseFileOnlyIterator(getRequiredSchemaReader(split.getBasePath().get()));
      if (projection != null) {
        baseIterator = new ProjectionIterator(baseIterator, projection);
      }
      this.iterator = new SkipMergeIterator(baseIterator, getLogFileIterator(split));
      projection = null;
      ...
      this.iterator = new MergeIterator(
          ...
          this.internalSchema1,
          projection,
          castMap,
          this.actualFieldTypes,
          this.requiredPos,
          this.emitDelete,
  ...
  private ClosableIterator<RowData> getLogFileIterator(MergeOnReadInputSplit split) {
    ...
    final HoodieMergedLogRecordScanner scanner = FormatUtils.logScanner(split, tableSchema, internalSchema1, conf, hadoopConf);
  ...
  private ClosableIterator<RowData> getUnMergedLogFileIterator(MergeOnReadInputSplit split) {
    ...
    final AvroToRowDataConverters.AvroToRowDataConverter avroToRowDataConverter =
        AvroToRowDataConverters.createRowConverter(tableState.getRequiredRowType());
    final FormatUtils.BoundedMemoryRecords records = new FormatUtils.BoundedMemoryRecords(split, tableSchema, internalSchema, hadoopConf, conf);
  ...
  static class MergeIterator implements RecordIterator {
    ...
    private final List<DataType> actualFieldTypes;
    ...
    MergeIterator(
        ...
        CastMap castMap,
        List<DataType> actualFieldTypes,
        int[] requiredPos,
        boolean emitDelete,
        int operationPos,
      ...
      this.projection = projection != null ? projection : RowDataProjection.instance(requiredRowType, requiredPos);
    ...
    @Override
    public boolean reachedEnd() throws IOException {
      RowDataCastProjection pr;
      if (castMap != null) {
        LogicalType[] objects = actualFieldTypes.stream().map(DataType::getLogicalType).toArray(LogicalType[]::new);
        pr = new RowDataCastProjection(objects, IntStream.range(0, objects.length).toArray(), castMap);
      } else {
        pr = null;
      }
      ...
      while (!readLogs && !this.reader.reachedEnd()) {
        ...
        if (scanner.getRecords().containsKey(curKey)) {
          ...
          if (pr != null) {
            currentRecord = pr.project(currentRecord);
          }
