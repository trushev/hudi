...
public class MergeOnReadInputFormat
  ...
  protected RecordIterator initIterator(MergeOnReadInputSplit split) throws IOException {
    if (schemaManager.isPresent()) {
      ...
      InternalSchema tableSchema = AvroInternalSchemaConverter.convertToEmpty(new Schema.Parser().parse(tableState.getAvroSchema()));
    ...
    if (!(split.getLogPaths().isPresent() && split.getLogPaths().get().size() > 0)) {
      if (split.getInstantRange().isPresent()) {
        ...
        BaseFileOnlyFilteringIterator it = new BaseFileOnlyFilteringIterator(
        ...
        return new ProjectionIterator(it, projection);
      } else {
        // base file only
        BaseFileOnlyIterator it = new BaseFileOnlyIterator(getRequiredSchemaReader(split.getBasePath().get()));
        Option<RowDataProjection> castProjection = getCastProjection();
        if (castProjection.isPresent()) {
          return new ProjectionIterator(it, castProjection.get());
        }
        return it;
      }
      ...
      Option<RowDataProjection> castProjection = getCastProjection();
      if (castProjection.isPresent()) {
        baseFileIterator = new ProjectionIterator(baseFileIterator, castProjection.get());
      }
      return new SkipMergeIterator(baseFileIterator, getLogFileIterator(split));
      ...
      Option<RowDataProjection> projectionBeforeMerge = schemaManager.map(sm -> {
        CastMap castMap = sm.getCastMap(querySchema, actualSchema);
        int[] positions = IntStream.range(0, actualFieldTypes.size()).toArray();
        return new RowDataCastProjection(FlinkInternalSchemaManager.project(actualFieldTypes, positions), positions, castMap);
      });
  ...
  private Option<RowDataProjection> getCastProjection(int[] positions) {
    if (schemaManager.isPresent()) {
      CastMap castMap = schemaManager.get().getCastMap(querySchema, actualSchema);
      if (castMap.containsAnyPos(requiredPos)) {
        return Option.of(new RowDataCastProjection(
            FlinkInternalSchemaManager.project(actualFieldTypes, requiredPos),
            positions,
